...
step <- as.numeric(readline(prompt = "Enter step: ")) #astral)))
# Задание сеточной поверхности
x=seq(from = dekart_begin, to = dekart_end, by = step)
y=x
# Задание двумерной функции на координатной сетке
...
dimnames(surface_matrix)=list(x, y)
write(paste("number of matrix elements:", nrow(surface_matrix)*ncol(surface_matrix)),"summary2.txt")
write(paste("number of rows:", nrow(surface_matrix)),"summary2.txt", append = T)
write(paste("number of cols:", ncol(surface_matrix)),"summary2.txt", append = T)
write(paste("sum of main diag elements:", sum(diag(surface_matrix))),"summary2.txt", append = T)
write(paste("row sums:",toString(as.vector(rowSums(surface_matrix)))),"summary2.txt", append = T)
write(paste("col sums:",toString(as.vector(colSums(surface_matrix)))),"summary2.txt", append = T)
...
surface_matrix <- outer(X = x,
                        ...
                        FUN = function(x,y) Re(exp(-1i * 0.5 * x * y)))
dimnames(surface_matrix) <- list(x, y)
write(paste("number of matrix elements:", nrow(surface_matrix)*ncol(surface_matrix)),"summary3.txt")
write(paste("number of rows:", nrow(surface_matrix)),"summary3.txt", append = T)
write(paste("number of cols:", ncol(surface_matrix)),"summary3.txt", append = T)
write(paste("sum of main diag elements:", sum(diag(surface_matrix))),"summary3.txt", append = T)
write(paste("row sums:",toString(as.vector(rowSums(surface_matrix)))),"summary3.txt", append = T)
write(paste("col sums:",toString(as.vector(colSums(surface_matrix)))),"summary3.txt", append = T)
